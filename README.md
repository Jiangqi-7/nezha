# neZha - Agent 专业角色切换框架

一个给 Agent 用的角色扮演框架，让 Agent 在不修改自身设定的情况下，随时切换到不同专业角色（律师、技术顾问、营销专家等）获取专业回复。

解决的核心问题：Agent 需要调用各种专业角色，但不想在自己身上定义太多 system prompt——通过 neZha 调用角色服务，角色服务返回专业回复给 Agent。
